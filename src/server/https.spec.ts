...
import { HttpsAppServer } from './https';


describe(HttpsAppServer.name, () => {
    let server: HttpsAppServer;
    let cert: Buffer;
    let key: Buffer;
    ...
    const __doHttps = async (method: Method, path: string, body?: string | Buffer, headers?: Headers) => {
        const port = server.addressInfo?.port;
        const url = `https://localhost:${port}${path}`;

        return await new Promise<string>((resolve, reject) => {
            const request = httpsClient.request(url, {
                method,
                headers,
    ...
    const httpsGet = (path: string, headers?: Headers) => {
        return __doHttps(Method.Get, path, undefined, headers);
    };

    const httpsPost = (path: string, body: string | Buffer, headers?: Headers) => {
        return __doHttps(Method.Post, path, body, headers);
    };

    const httpsPostJson = (path: string, data: any, headers?: Headers) => {
        ...
        return httpsPost(path, body, headers);
    };

    beforeEach(() => {
        cert = fs.readFileSync('./test/cert');
        key = fs.readFileSync('./test/key');
        server = new HttpsAppServer();
    });
    ...
    it('should listen and resolve an incoming get request', async () => {
        ...
        server.app(app);
        await awaitListening();
        const responseBody = await httpsGet('/app/status');
    ...
    it('should listen and resolve an incoming post request', async () => {
        ...
        const responseBody = await httpsPostJson('/app/status', message);
